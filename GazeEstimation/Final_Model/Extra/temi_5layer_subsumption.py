...
Notes week 10 meeting.

quantify where it stops working, 

1.8m is a useful target for distance...

its better closer and then works further. Hypothesis... moving targets (move 1.5 to 3. or 3m to 1.5m)

make a video rather than live camera.

students T tests (determine any meaningful differences in different samples). If something different, then why?

segment anything 2... (SAM / SamurAI)
...
MIN_PERSON_CONF = 0.3  # For person detection as backup
MIN_FACE_CONF = 0.05   # Very low confidence for distant face detection
MAX_NUM_FACES = 1
MIN_DETECTION_CONFIDENCE = 0.15  # Lowered for distant faces when we have face ROI
...
class FaceDetector:
    ...
    def __init__(self, model_path: str = FACE_YOLO_MODEL, min_conf: float = MIN_FACE_CONF):
        try:
            ...
            print(f"Warning: Face detector not available ({e}). Using person detector fallback.")
            self.model = None
            self.available = False
        self.base_min_conf = min_conf
    
    def detect_faces(self, frame, adaptive_conf: bool = True) -> List[Tuple[int,int,int,int,float]]:
        """Return face boxes [(x1,y1,x2,y2,conf), ...] using face-specific YOLO model."""
        ...
        for det in res.boxes.data.tolist():
            ...
            # yolov8n-face.pt typically has class 0 for faces
            # Accept any class from face model since it's face-specific
            face_area = (x2 - x1) * (y2 - y1)
            face_area_fraction = face_area / frame_area
            
            # Ultra-aggressive confidence thresholds for distant faces
            if adaptive_conf:
                if face_area_fraction > 0.015:  # Medium-large face
                    min_conf = self.base_min_conf
                elif face_area_fraction > 0.008:  # Small face  
                    min_conf = max(0.03, self.base_min_conf - 0.1)
                elif face_area_fraction > 0.003:  # Very small face
                    min_conf = max(0.02, self.base_min_conf - 0.15)
                else:  # Extremely small face (distant)
                    min_conf = max(0.01, self.base_min_conf - 0.2)
            else:
                min_conf = self.base_min_conf
            
            # Also check minimum pixel size
            face_width = x2 - x1
            face_height = y2 - y1
            min_size_ok = face_width >= 20 and face_height >= 20  # Very small minimum
            
            if conf >= min_conf and min_size_ok:
                out.append((int(x1), int(y1), int(x2), int(y2), float(conf)))
        
        # Sort by confidence, return best faces first
        out.sort(key=lambda x: x[4], reverse=True)
        return out[:self.max_faces] if hasattr(self, 'max_faces') else out
...
class FacePoseEstimator:
    """Face-first pose estimation: YOLO face detection -> MediaPipe landmarks -> pose estimation."""
    ...
    def estimate(self, frame, estimated_distance: Optional[float] = None, 
        ...
        """Face-first estimation: Detect faces with YOLO, then apply MediaPipe landmarks.
        
        Pipeline: Face YOLO -> MediaPipe landmarks -> Pose estimation
        """
        H, W = frame.shape[:2]
        
        # Step 1: Detect faces using YOLO face model (primary method)
        face_boxes = self.face_detector.detect_faces(frame, adaptive_conf=True)
        
        if face_boxes:
            # Use the most confident face detection
            best_face = face_boxes[0]  # Already sorted by confidence
            face_box = best_face[:4]
            face_conf = best_face[4]
            
            print(f"Face detected: confidence={face_conf:.3f}, box={face_box}")
            
            # Try MediaPipe landmarks on detected face region with progressively enhanced preprocessing
            ...
            # If MediaPipe fails on face region, use geometric landmark estimation
            print(f"MediaPipe failed on face region, using geometric estimation")
            ...
            if result and result.get('ok'):
                result['face_detection_confidence'] = face_conf
                result['face_box'] = face_box
                result['detection_method'] = "face_yolo_geometric"
                return result
        
        # Step 2: Fallback to person detection if no faces found
        if person_boxes:
            print("No faces detected, trying person box fallback")
            ...
            result = self._estimate_pose_from_landmarks(pts2d, face_kps, estimated_distance, W, H)
            if result and result.get('ok'):
                result['detection_method'] = "person_box_geometric"
                return result
        
        print("All detection methods failed")
        return None  # All approaches failed
    
    def _try_mediapipe_on_face_region(self, frame, face_box: Tuple[int,int,int,int], 
                                     estimated_distance: Optional[float] = None,
                                     enhancement_level: int = 0) -> Optional[dict]:
        """Try MediaPipe detection on a specific face region with optional preprocessing."""
        H, W = frame.shape[:2]
        x1, y1, x2, y2 = face_box
        
        # Expand face region slightly for better landmark detection
        ...
        # Adapt MediaPipe confidence for face region
        if estimated_distance is not None:
            new_confidence = get_adaptive_detection_confidence(estimated_distance)
        else:
            # Lower confidence since we already have face detection
            new_confidence = 0.1
        ...
        for idx in LANDMARK_IDS:
            # Landmark coordinates in face region
...
def main():
    ...
    # Initialize detection layers (for Layer 4 - Gaze Detection)
    ...
    print("TEMI 5-LAYER SUBSUMPTION ARCHITECTURE RUNNING (Face-First Detection)")
    ...
    print("Layer 4: Face-First Gaze Detection (YOLO Face -> MediaPipe landmarks -> Pose)")
    ...
    while cap.isOpened():
        ...
        # -----------------------
        # LAYER 4: GAZE DETECTION (Face-first approach)
        # -----------------------
        predictions = []
        
        # Face-first detection: No person detection needed
        # Enhanced cascade face pose estimation
        pose = face_estimator.estimate(frame)
        ...
        if pose and pose.get('ok', False):
            ...
            face_box = pose.get('face_box')  # YOLO face detection box

            # Draw face detection box if available
            if face_box:
                x1, y1, x2, y2 = face_box
                face_conf = pose.get('face_detection_confidence', 0.0)
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 255), 2)  # Cyan for face
                cv2.putText(frame, f"Face {face_conf:.2f}", (x1, y1 - 6),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

            # Draw face landmarks
            for pt in face_kps:
                x, y = int(pt['x']), int(pt['y'])
                cv2.circle(frame, (x, y), 3, (0, 255, 0), -1)  # Smaller landmarks

            # Calculate face bounding box from landmarks (for proximity gate)
            xs = [pt['x'] for pt in face_kps]
            ys = [pt['y'] for pt in face_kps]
            x_min, x_max = int(min(xs)), int(max(xs))
            y_min, y_max = int(min(ys)), int(max(ys))
            
            # Use detected face box if available, otherwise use landmark bounds
            if face_box:
            ...
            face_dict = {
                'face': {
                    ...
                    'confidence': pose.get('face_detection_confidence', 0.0),
                    'class': 'face',
        ...
        interaction_state = layer3_approaching.update_interaction(
            person_detected=bool(pose and pose.get('ok')),  # Use face detection as person indicator
        ...
        cv2.imshow("Temi 5-Layer (Face-First Detection)", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
